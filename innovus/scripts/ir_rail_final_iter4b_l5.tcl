# ==========================================================================
# ir_rail_final_iter4b_l5.tcl
# Final TSMC 28 nm static IR / rail analysis for the L=5 operating mode.
# --------------------------------------------------------------------------
# Flow:
#   1. Restore the final Iter4B routed implementation.
#   2. Read the L=5 gate-level SAIF activity.
#   3. Generate fresh SAIF-based static VDDC/VSSC current data.
#   4. Generate/reuse a run-local 28 nm tech-only PGV model.
#   5. Run Voltus static rail analysis on the VDDC/VSSC core power domain.
#
# The analysis uses:
#   final DB : final_hold_eco_iter4b_trial
#   activity : core_L5.saif
#   corner   : SlowView, SS 0.81 V, 125 C
#   RC       : cworst
#   scope    : project-level VDDC/VSSC core-domain rail analysis
#
# Pad-ring sections outside I0 are retained in the integrity reports as a
# documented tech-only-PGV modeling limitation.
#
# All outputs are written under:
#   innovus/work/ir_signoff_iter4b_l5_final/
# ==========================================================================

set PSD ./ir_signoff_iter4b_l5_final
file mkdir $PSD

puts "===== FINAL ITER4B L5 IR/RAIL: RESTORE ====="
restoreDesign ../dataout/design_saves/final_hold_eco_iter4b_trial.dat top
puts "FINAL_IR_DB_RESTORED=[dbGet top.name]"
...
set PGV_DIR     $PSD/pgv_techonly
set PGV_PREFIX  pgv28
set VSRC_SEARCH 60       ;# um; snap ideal source to the nearest VDDC/VSSC node
# Final activity source: L5 gate-level SAIF on the final Iter4B database.
set ACT_NOTE "L5 gate-level SAIF (core_L5.saif, scope design_tb/dut -> block I0)"

set POWER_OUT $PSD/power
file mkdir $POWER_OUT

set I_VDD $POWER_OUT/static_${VDDC_NET}.ptiavg
set I_VSS $POWER_OUT/static_${VSSC_NET}.ptiavg


# ---------------- Final L5 SAIF current generation ----------------
set SAIF_FILE [file normalize ../datain/saif/core_L5.saif]
set SAIF_SCOPE design_tb/dut
set CORE_INST I0

inote "Final Iter4B L5 current generation"
inote "SAIF_FILE=$SAIF_FILE"
inote "SAIF_SCOPE=$SAIF_SCOPE CORE_INST=$CORE_INST"

if {![file exists $SAIF_FILE]} {
    puts stderr "ERROR: missing L5 SAIF: $SAIF_FILE"
    exit 2
}

# Restore / confirm final analysis environment.
if {[catch { set _v [get_analysis_views] } e] || $_v eq ""} {
    catch { read_mmmc ../datain/mmmc.view }
}
catch { set_analysis_view -setup SlowView -hold FastView }

# Reassert PG connectivity for the restored final DB.
catch { globalNetConnect VDDC -type pgpin -pin VDD    -instanceBasename * -override }
catch { globalNetConnect VSSC -type pgpin -pin VSS    -instanceBasename * -override }
catch { globalNetConnect VDDP -type pgpin -pin VDDPST -instanceBasename * -override }
catch { globalNetConnect VSSP -type pgpin -pin VSSPST -instanceBasename * -override }
catch { globalNetConnect POC  -type pgpin -pin POC    -instanceBasename * -override }
catch { applyGlobalNets }

catch { setAnalysisMode -analysisType onChipVariation -cppr both }

if {[catch { extractRC } e]} {
    inote "extractRC WARN: $e"
}

# Configure final SAIF-based static power analysis.
catch { set_power_analysis_mode -reset }
catch { set_power_output_dir -reset }
catch { set_power_output_dir $POWER_OUT }
...
if {[catch {
    ...
    puts stderr "ERROR: set_power_analysis_mode failed: $e"
    exit 3
}
...
if {[catch {
    ...
    inote "read_activity_file with -block failed: $e"
    if {[catch {
        read_activity_file \
            -format SAIF \
            -scope $SAIF_SCOPE \
        ...
        puts stderr "ERROR: L5 SAIF annotation failed: $e2"
        exit 4
    }
}

inote "L5 SAIF activity annotation complete"

# VS format explicitly generates the current data consumed by rail analysis.
if {[catch {
    report_power \
        -rail_analysis_format VS \
        -outfile $POWER_OUT/chip_L5_vs.rpt
} e]} {
    puts stderr "ERROR: L5 VS-format report_power failed: $e"
    exit 5
}

set POWER_DB $POWER_OUT/power.db

if {![file exists $I_VDD] || ![file exists $I_VSS]} {
    puts stderr "ERROR: final L5 static current files were not generated"
    puts stderr "Expected: $I_VDD"
    puts stderr "Expected: $I_VSS"
    exit 6
}

inote "L5 VDDC current file generated: $I_VDD"
inote "L5 VSSC current file generated: $I_VSS"
inote "L5 power DB: $POWER_DB"

set IRSUM [open $PSD/ir_summary.txt w]
proc irs {m} { global IRSUM; puts $IRSUM $m; flush $IRSUM }
irs "======================================================================"
irs " FINAL ITER4B L5 SAIF IR / RAIL ANALYSIS   design=[dbGet top.name]"
...
if {!$PGV_OK} {
    ...
    inote "SKIP rail analysis: static current files missing ($I_VDD / $I_VSS). Generate the final L5 SAIF currents first."
    irs "RAIL: SKIPPED (final L5 SAIF static current files missing)"
...
if {$RAIL_OK} {
    ...
    if {$_rd ne ""} {
        ...
        irs ""
        irs "CORE-DOMAIN IR (worst node within analyzed SlowView, ss 0.81V, techonly PGV, activity=$ACT_NOTE):"
...
puts "##### FINAL_ITER4B_L5_IR_RUN_COMPLETE #####"
exit
